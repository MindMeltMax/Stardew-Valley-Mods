...
namespace ManyEnchantments
{
    ...
    public class ModEntry : Mod
    {
        ...
        {
            if (item is not null and Tool t)
            {
        ...
        [HarmonyPrefix]
        public static bool GetForgeCost_Prefix(ForgeMenu __instance, Item left_item, Item right_item, ref int __result)
        {
            try
            {
                if (right_item != null && Utility.IsNormalObjectAtParentSheetIndex(right_item, 72))
                {
                    if (left_item is not null and Tool t)
        ...
        public static bool ReceiveLeftClick_Prefix(ForgeMenu __instance, int x, int y)
        {
            try
            {
                if (__instance.unforgeButton.containsPoint(x, y) && __instance.rightIngredientSpot.item == null && __instance.IsValidUnforge())
                {
                    if (__instance.leftIngredientSpot.item is MeleeWeapon weapon)
                    {
                    ...
                    else if (__instance.leftIngredientSpot.item is Tool tool)
                    {
                        bool has_enchantment = false;
        ...
        [HarmonyPrefix]
        public static bool IsValidCraftIngredient_Prefix(Item item, ref bool __result)
        {
        ...
        [HarmonyPrefix]
        public static bool HighlightItems_Prefix(Item i, ref bool __result)
        {
        ...
        [HarmonyPrefix]
        public static bool IsValidUnforge_Prefix(ForgeMenu __instance, ref bool __result)
        {
            try
            {
                if (__instance.rightIngredientSpot.item == null && __instance.leftIngredientSpot.item is not null and MeleeWeapon weapon)
                {
        ...
        public static bool Forge_Prefix(Tool __instance, Item item, ref bool __result)
        {
            try
            {
                ...
                if (enchantment is not null and DiamondEnchantment)
                {
