...
namespace SoundsPatcher.Utility
{
    public class Utilities
    {
        ...
        {
            Dictionary<string, bool> soundDict = [];

            string[] cues = SoundCues.Split('/');
            cues = [.. cues.OrderBy(x => x)];
        ...
        public static void UpdateSoundsDict(Dictionary<string, bool> original)
        {
            string[] cues = SoundCues.Split('/');
            cues = [.. cues.OrderBy(x => x)];
            ...
            foreach (string cue in cues)
            {
                if (!original.ContainsKey(cue))
                    original.Add(cue, false);
                if (ModEntry.IConfig.UnknownSounds.ContainsKey(cue))
                    ModEntry.IConfig.UnknownSounds.Remove(cue);
            }    
        }

        public static Dictionary<string, bool> GetSongsDict()
        {
            Dictionary<string, bool> songDict = [];

            string[] songs = SongCues.Split('/');
            songs = [.. songs.OrderBy(x => x)];
