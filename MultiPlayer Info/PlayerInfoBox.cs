            ...
            switch (config.Position)
            {
                ...
                case BoxPosition.BottomRight:
                    if (config.ShowPlayerIcon)
                    {
                        ...
                        b.DrawString(Game1.smallFont, $"{Math.Round(info.Stamina)}/{info.MaxStamina}", new(xPositionOnScreen - PlayerIconBoxSize + 10, yPositionOnScreen + 4 + 44), staminaColor, 0f, Vector2.Zero, 1f, SpriteEffects.None, .89f);
                        return;
                    }
